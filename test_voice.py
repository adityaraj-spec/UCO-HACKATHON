...
# Import PhaseGuard Layer 1 V2 model definition
...
def main():
    ...
    model_path = "models/layer1_v2_best.pth"
    if not os.path.exists(model_path):
        print(f"ERROR: Trained model weights not found at: {model_path}")
        print("Please run retrain_v2.py first to train the model.")
        sys.exit(1)
        
    print(f"Loading PhaseGuard Layer 1 V2 model weights from {model_path}...")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = PhaseGuardL1_V2()
    ...
    try:
        ...
        # Convert to 2-channel Mel-Spectrogram (Magnitude + Phase derivative)
        from rebuild_dataset import audio_to_dual_channel_spectrogram
        spec_2ch = audio_to_dual_channel_spectrogram(audio)
        
        # Prepare tensor (shape: 1, 2, 128, 128)
        tensor = torch.FloatTensor(spec_2ch).unsqueeze(0).to(device)
